...
针对大模型在处理含有 API Key、访问令牌、密码、数据库连接串等敏感凭据时的隐私泄漏风险，本模型用于在网关层执行**单步 Logits 二分类判别（`SECRET` vs `SAFE`）**，结合确定性规则与本地占位符映射（`<PRIV_xxx_SECRET_N>`），实现**敏感数据在离开本地网络前透明脱敏、流式回包时本地透明还原**。
...
1. **零自回归采样，直接读取末位 Logits**：
   ...
   - **单步前向超低延迟**：仅需单次前向（Forward Pass），边缘设备（如 Intel N100）上推理耗时仅 **~20-40ms**（Transformers）。
2. **全线性层微调与输出层解绑**：
   - 采用 LoRA 微调所有线性层，并专门微调了输出投影层 `lm_head`（rank=16, alpha=32）。
   - 在合并时解绑 `lm_head`（`tie_word_embeddings = False`），极大增强了对于专有分类目标 Token 的判别置信度与区分度。
3. **真实基准评测表现（Empirical Metrics）**：
   - **IID 测试集** (250 docs, 880 candidates):
     - Recall: **100.0%** (10/10 漏报全部修复)
     - Precision: **100.0%**
     - F1-score: **1.0000**
     - INVALID: **0**
   - **OOD 域外泛化测试集** (250 docs, 848 candidates):
     - Recall: **89.27%** (相比自回归生成提升近 3 个百分点)
     - Precision: **99.57%** (仅 1 例误报)
     - F1-score: **0.9414**
     - INVALID: **0**
...
print(classify_credential("sk-abcdefghijklmnopqrstuvwxyz012345"))
print(classify_credential("今天天气晴朗适合散步"))
```

完整网关（正则 Layer 0 + 本模型残差 + Vault 还原）见 GitHub 仓库，不要把整段对话丢进 0.5B。

---

## 🛡️ 隐私网关分层落地

- **Layer 0（确定性硬规则）**：PEM、已知前缀 Key、JWT、数据库 URI 密码，纯正则拦截。
- **Layer 1（候选词抽取）**：未命中 Layer 0 的赋值 / Bearer / 高熵串。无候选则 0ms。
- **Layer 2（0.5B Logits 判别）**：仅对未知候选做单步判定。
